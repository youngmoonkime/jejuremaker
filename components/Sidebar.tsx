...
const Sidebar: React.FC<SidebarProps> = ({ 
    ...
    isSuperAdmin, userTokens, setUserTokens 
}) => {
    ...
    return (
        <aside className="w-72 flex-shrink-0 hidden lg:flex flex-col sticky top-24 h-[calc(100vh-8rem)] pr-2">
            ...
            <div className="flex-shrink-0 flex flex-col gap-6 pr-2 max-h-[65%] overflow-y-auto scrollbar-hide">
                ...
                {isSuperAdmin && (
                    <div className="flex-shrink-0 p-5 rounded-3xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 shadow-sm relative overflow-hidden group">
                        <div className="flex items-center gap-2 mb-3 text-indigo-600 dark:text-indigo-400 font-bold">
                            ...
                            <span className="text-xs uppercase tracking-wider">Super Admin Control</span>
                        </div>
                        <div className="space-y-3">
                            <div className="flex flex-col gap-1">
                                <label className="text-[10px] font-bold text-indigo-500 uppercase">Manual Token Set</label>
                                <div className="flex gap-2">
                                    <input 
                                        type="number" 
                                        className="w-full bg-white dark:bg-gray-800 border border-indigo-200 dark:border-indigo-700 rounded-lg px-3 py-1.5 text-sm font-bold focus:ring-2 focus:ring-indigo-500 outline-none"
                                        placeholder="Tokens..."
                                        onKeyDown={(e) => {
                                            if (e.key === 'Enter' && setUserTokens) {
                                                const val = parseInt((e.target as HTMLInputElement).value);
                                                if (!isNaN(val)) setUserTokens(val);
                                            }
                                        }}
                                    />
                                    <button 
                                        onClick={(e) => {
                                            ...
                                            if (input && setUserTokens) {
                                                const val = parseInt(input.value);
                                                if (!isNaN(val)) setUserTokens(val);
                                            }
                                        }}
                                        className="bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-indigo-700 transition-colors whitespace-nowrap"
                                    >
                                        Set
                                    </button>
                                </div>
                            </div>
                            <p className="text-[10px] text-indigo-400 font-medium leading-tight">관리자 전용 토큰 설정 도구입니다. 엔터 또는 Set 클릭 시 즉시 반영됩니다.</p>
                        </div>
                    </div>
                )}
